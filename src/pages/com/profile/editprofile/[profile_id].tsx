...
export default function EditProfile() {
  ...
  const submitData = async (data: any) => {
    ...
    const newKonsultanData = {
      konsultanId: profile_id as string,
      konsultanName: data.company_name,
      konsultanPIC: `${data.pic_fullname}`,
      konsultanEmail: data.business_email,
      konsultanLocation: data.company_address,
      konsultanAddress: data.company_address,
      konsultanPhoneNumber: data.company_phone,
      konsultanRangeTotalEmployees: data.employee_number,
      konsultanDescription: data.company_description,
    };

    try {
      const getKonsultan = await API.graphql<GraphQLQuery<GetKonsultanQuery>>(
        graphqlOperation(queries.getKonsultan, {
          konsultanId: profile_id,
        })
      );
      ...
      let newKonsultan;
      if (getKonsultan.data && getKonsultan.data.getKonsultan) {
        newKonsultan = await API.graphql<GraphQLQuery<UpdateKonsultanMutation>>(
          {
            query: mutations.updateKonsultan,
            variables: { input: newKonsultanData },
          }
        );
      } else {
        newKonsultan = await API.graphql<GraphQLQuery<CreateKonsultanMutation>>(
          {
            query: mutations.createKonsultan,
            variables: { input: newKonsultanData },
          }
        );
      }
